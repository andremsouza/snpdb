...
for nsnps in nspns_list:
    ...
    for nsamples in nsamples_list:
        ...
        results[experiment_id][size_id]['sample']['dbsize'] = [
        ]  # doc size in db
        results[experiment_id][size_id]['sample']['time'] = [
        ]  # insertion time
        ...
        for i in range(N):
            ...
            results[experiment_id][size_id]['sample']['dbsize'].append(
                sample_size)
            results[experiment_id][size_id]['dbsize'].append(map_size +
                                                             sample_size)
...
for nsnps in nspns_list:
    ...
    for nsamples in nsamples_list:
        ...
        for i in range(N):
            ...
            t = time.time()
            snpdb.import_samples(
                sample_reader=readers.PlinkSampleReader(pfname),
                map_name=experiment_id + size_id,
                id_map=id_map,
                report=False)
            t = time.time() - t
